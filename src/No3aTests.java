...
public class No3aTests {
	...
	@Before
	public void setUp() {
		graph = new DirectedAdjacencyListGraph<String, Integer>(10);
	}
	...
	public void testAddNewEdgeAndGetEdge() {
		...
		for (int v1 = 0; v1 <= 9; v1++) {
			for (int v2 = 0; v2 <= 9; v2++) {
				if (v1 != v2) {
					...
					assertEquals("Number of edges not correct", edgeCount, graph.numEdges());
					assertEquals("getEdge gave back wrong edge", (Integer) edgeCount, graph.getEdge(v1, v2));
				}
			}
		}
		...
		for (int v1 = 0; v1 <= 9; v1++) {
			for (int v2 = 0; v2 <= 9; v2++) {
				if (v1 == v2) {
					assertNull("Graph should not have self-edges", graph.getEdge(v1, v2));
				} else {
					edgeCount++;
	...
	public void testAddEdgeReplacesExistingEdge() {
		...
		assertEquals("setEdge did not replace existing edge", (Integer) 2000, graph.getEdge(3, 7));
		assertEquals("setEdge replaced incorrect edge", (Integer) 1001, graph.getEdge(7, 3));
		assertEquals("Incorrect number of vertices", 10, graph.numVertices());
